...
async function summarizeText(text) {
  const prompt = `redacta un texto de la siguiente idea. sin agregar explicaciones ni saludos, ni despedidas, ni opiniones.
Texto:
${text}`;
...
document.getElementById('generate-btn').addEventListener('click', async () => {
  ...
  if (!userInput) {
    resultEl.textContent = 'write something to generate.';
    return;
  }

  resultEl.textContent = 'Detect language...';

  try {
    const detectedLanguage = await detectLanguage(userInput);
    resultEl.textContent = `Detected language: ${detectedLanguage}. thinking...`;
